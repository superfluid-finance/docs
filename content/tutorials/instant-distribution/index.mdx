...
Now that we have an index, lets specify it's behavior. In this tutorial, bob wants to send funds to both alice and dan. He also wants alice to receive twice the amount that dan receives.

![](/img/instant-distribution-42.png)

We do this by creating a **subscription** for each recipient. Let's go ahead and `updateSubscription()` for alice.
...
1. We again use `callAgreement()`
2. We specify we want an **Instant Distribution Agreement** by selecting `sf.agreements.ida.address`
3. Using the method `updateSubscription()`, we pass the arguments for the DAIx token, `indexId` "42".
