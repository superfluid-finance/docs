...
3. Using the method `createIndex()`, we pass the arguments for the DAIx token, and `indexId` "42".

> A publisher can create virtually unlimited indexes.

Now that we have an index, lets specify it's behavior. In this tutorial, bob wants to send funds to both alice and dan. He also would like alice to receive twice the amount that dan receives, whenever a distribution occurs.

![](/img/instant-distribution-42.png)

We do this by creating a **subscription** for each recipient. Let's go ahead and call `updateSubscription()` for alice.
...
1. We again use `callAgreement()` and specify this is an IDA by using `sf.agreements.ida.address`
2. Using the method `updateSubscription()`, we pass the arguments for the DAIx token, `indexId` "42", `recipient` alice, and an amount of `units` "200".

Let's do the same for dan, but for "100" `units`. 
