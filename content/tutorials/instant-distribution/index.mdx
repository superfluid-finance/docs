...
In the previous step, we created two subscriptions for bob's Publishing Index. Now the recipients have the option to **approve subscription**. By doing so, their DAIx balance will be updated instantly when an Instant Distribution occurs. If they do not approve it, they will still receive the funds, but they may have to perform an additional transaction.
...
Now we perform an Instant Distribution using `updateIndex()` with a value of **0.01** (units of ether).
...
1. We again use `callAgreement()` and specify this is an IDA by passing `sf.agreements.ida.address`
2. Using the method `updateIndex()`, we pass the arguments for `token` DAIx, `indexId` "42", and the desired `amount`.
